...
Surface is intentionally small: six tools (`check_setup`, `read_config`, `resolve_config`, `validate_config`, `dry_run`, `write_config`) plus one resource (`renovate://presets`). Don't grow this without a reason — the roadmap for expansion lives in the GitHub issues, not in ad-hoc additions.
...
**`resolve_config` is offline and never shells out.** It expands `extends` against the committed preset catalogue via `src/lib/presetResolver.ts`. External presets (`github>`, `gitlab>`, `local>`, npm) are returned in `presetsUnresolved` with a clear reason rather than fetched. The resolver is purely in-process — don't route it through `renovateCli.ts`, and don't add a network code path without first discussing scope (the `externalPresets: true` opt-in sketched in issue #1 is future work, not v1).
