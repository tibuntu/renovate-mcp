...
**File selection (`managerFilePatterns`):** entries are matched against POSIX-style paths relative to `repoPath` with the semantics of Renovate's `matchRegexOrGlobList`: `*` matches everything; an entry wrapped as `/…/` (or `/…/i` for case-insensitive) is a regex; anything else is a glob evaluated with `minimatch` using `{ dot: true, nocase: true }` (so `**/*.yml` also matches `.github/workflows/ci.yml`, and `dockerfile` matches `Dockerfile`); a leading `!` makes an entry negative. A path matches when it matches at least one positive entry (if there are any) and every negative entry — `["**/*.yaml", "!**/vendor/**"]` selects every YAML file outside `vendor/`. Regex entries still run in the worker under `matchTimeoutMs`; globs are evaluated in-process. An invalid `/…/` regex is reported as an error rather than silently treated as a glob. Note that Renovate's own extract phase (`workers/repository/extract/file-match.js`) evaluates each entry on its own and unions the results, so a `!` entry there adds the complement instead of excluding — confirm negations with `dry_run`.
